...
export async function seedReadyIngredients() {
    console.log("Seeding Ready to Eat ingredients from CSV...");

    try {
        // Read CSV file
        const csvFilePath = path.resolve(__dirname, "../../data/IngredientsReady.csv");
        ...
        // Find Ready to Eat category
        const category = await prisma.ingredientCategory.findFirst({
            where: { name: "Ready to Eat" },
        });

        if (!category) {
            console.error("Category 'Ready to Eat' not found. Please run category seeding first.");
            return [];
        }
        ...
        console.log(`Successfully seeded ${createdIngredients.length} Ready to Eat ingredients.`);
        ...
        console.error("Error seeding Ready to Eat ingredients:", error);
        throw error;
    } finally {
