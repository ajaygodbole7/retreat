...
export async function seedSpicesDryIngredients() {
    console.log("Seeding Spices(Dry) ingredients from CSV...");

    try {
        // Read CSV file
        const csvFilePath = path.resolve(__dirname, "../../data/IngredientsSpicesDry.csv");
        ...
        // Find Spices Dry category
        const category = await prisma.ingredientCategory.findFirst({
            where: { name: "Spices (Dry)" },
        });

        if (!category) {
            console.error("Category 'Spices (Dry)' not found. Please run category seeding first.");
            return [];
        }
        ...
        for (const record of validRecords) {
            ...
            try {
                ...
                // Find the appropriate unit based on US Units column
                let unitToUse = null;
                if (metricUnit) {
                    ...
                    if (unitToUse) {
                        console.log(`Found matching unit '${unitToUse.name}' for US unit '${metricUnit}'`);
                    } else {
                        console.log(`No exact match found for US unit '${metricUnit}', trying partial match...`);
                ...
                const ingredient = await prisma.ingredient.create({
                    data: {
                        ...
                        shelfLifeDays: 365, // Default 1 year for spices
        ...
        console.log(`Successfully seeded ${createdIngredients.length} Spices (Dry) ingredients.`);
        ...
        console.error("Error seeding Spices (Dry) ingredients:", error);
        throw error;
    } finally {
