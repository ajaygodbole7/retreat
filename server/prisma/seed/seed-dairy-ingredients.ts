...
export async function seedDairyIngredients() {
    console.log("Seeding Spices(Dry) ingredients from CSV...");

    try {
        // Read CSV file
        const csvFilePath = path.resolve(__dirname, "../../data/IngredientsDairy.csv");
        ...
        if (!category) {
            console.error("Category 'Dairy' not found. Please run category seeding first.");
            return [];
        }
