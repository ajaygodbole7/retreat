...
export async function seedFrozenVegIngredients() {
    console.log("Seeding Frozen vegetables ingredients from CSV...");

    try {
        // Read CSV file
        const csvFilePath = path.resolve(__dirname, "../../data/IngredientsFrozenVeg.csv");
        ...
        // Find Frozen Vegetables category
        const category = await prisma.ingredientCategory.findFirst({
            where: { name: "Frozen Vegetables" },
        });

        if (!category) {
            console.error("Category 'Frozen Vegetables' not found. Please run category seeding first.");
            return [];
        }
        ...
        console.log(`Successfully seeded ${createdIngredients.length} Frozen Vegetables ingredients.`);
        ...
        console.error("Error seeding Frozen Vegetables ingredients:", error);
        throw error;
    } finally {
